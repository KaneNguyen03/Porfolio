...
const YouTubeEmbed = ({ url, title }: { url: string; title: string }) => {
  ...
  try {
    ...
    startTime = urlObj.searchParams.get("t")?.replace("s", "") || "0";
  } catch {
    return null;
  }
  ...
  if (isPlaying) {
    return (
      <iframe
        src={`https://www.youtube-nocookie.com/embed/${videoId}?start=${startTime}&autoplay=1&rel=0&modestbranding=1`}
        title={title}
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="w-16 h-16 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300 border border-white/20 shadow-2xl">
...
const ProjectsPage = () => {
  ...
  const [showMoreResponsibilities, setShowMoreResponsibilities] = useState<
    string | null
  >(null);
  ...
  const filteredProjects =
    selectedFilter === "All"
      ...
      : projects.filter((project) =>
          project.technologies.map(normalizeTech).includes(selectedFilter),
        );
      ...
      <div className="container-width">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="text-center mb-20">
            ...
            >
              A selection of shipped work, spanning full-stack products and
              backend systems.
            </motion.p>
            ...
            {filteredProjects.map((project) => (
              ...
              >
                <Card className="relative overflow-hidden group transition-all duration-300 flex flex-col h-full border border-slate-200/70 dark:border-slate-800/70">
                  <div
                    aria-hidden="true"
                    className="absolute -top-24 -right-24 h-64 w-64 rounded-full bg-(--brand-hero) blur-3xl opacity-10 group-hover:opacity-15 transition-opacity"
                  />
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                      {project.technologies
                                        ...
                                        .map((tech) => (
                                          <span
                                            key={tech}
                                            className="px-2 py-1 bg-white/10 rounded-lg text-xs font-medium"
                                          >
                                            {tech}
                                          </span>
                                        ))}
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-gray-200 dark:border-gray-700 shrink-0 mt-auto">
                        <div className="flex flex-1 gap-3">
                          ...
                          !project.liveDemo.includes("youtube.com") ? (
                            ...
                            <Button
                              disabled
                              className="w-full rounded-2xl h-12 flex-1"
                            >
                              <ExternalLink size={16} />
